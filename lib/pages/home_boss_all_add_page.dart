...
class _HomeBossAllAddPageState extends State<HomeBossAllAddPage>
  ...
  Future<bool> initData() {
    return AppApi.ins().obtainOperationPhoto().flatMap((value) {
      mLabels = CacheProvider.getIns().getAllLabel();
      operationEntity = value;

      return AppApi.ins().obtainGetAllBossList();
  ...
  Future<bool> errorLoadData() {
    return AppApi.ins().obtainGetLabelList().onErrorReturn([]).flatMap((value) {
      ...
      return AppApi.ins().obtainOperationPhoto();
    }).flatMap((value) {
      operationEntity = value;
  ...
  void loadData() {
    AppApi.ins().obtainGetAllBossList().listen((event) {
      mData = event;
  ...
  void doBatchTack() {
    if (!mSelectList.isNullOrEmpty()) {
      ...
      AppApi.ins().obtainFollowBossList(mSelectList).listen((event) {
        ...
        CacheProvider.getIns().insertBatchBoss(list);
