...
class SpeechTackAllPage extends StatefulWidget {
  String mLabel;

  SpeechTackAllPage(this.mLabel, {Key key}) : super(key: key);

  @override
  _SpeechTackAllPageState createState() => _SpeechTackAllPageState();
}

class _SpeechTackAllPageState extends State<SpeechTackAllPage>
    with
  ...
  Future<dynamic> futureData() {
    return DataConfig.getIns().fromSplash ? initAllData() : initLogoutData();
  }
  ...
  Future<dynamic> initAllData() {
    ...
    var mPage = CacheProvider.getIns().getAllArticle();

    return Observable.just(mPage).doOnData((event) {
      pageParam.next(1);
      totalArticleNumber = event.total;
      hasData = event.hasData;
      concat(event.records, false);
    ...
    if (!loadMore) {
      ...
      AppApi.ins()
          ...
          .listen((event) {
            ...
            concat(event.records, false);
            titleText =
                event.records[0]?.recommendType == "0" ? "最近更新" : "为你推荐";
